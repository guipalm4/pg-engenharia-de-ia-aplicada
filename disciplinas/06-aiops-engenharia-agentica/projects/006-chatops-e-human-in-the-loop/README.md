...
- [x] **`chatops.py` trazia um hack de `sys.path` do material original que aqui não fazia nada.** O lab calcula `PROJECT_ROOT = dirname(__file__)/".."` porque no repositório do professor os labs vivem em `labs/`; nesta trilha cada aula é uma pasta com o entrypoint na raiz, então o caminho apontava para `projects/` — um diretório sem `core/` nem `tools/`. Os imports funcionavam mesmo assim porque o Streamlit insere o diretório do script no `sys.path` antes de executá-lo. A linha não era só inútil: ela colocava o diretório **errado** na posição 0 do `sys.path`, à frente do diretório certo. Foi corrigida em toda a trilha quando a aula 007 passou a usar a variável para montar caminho de arquivo e o defeito deixou de ser latente. **Código de caminho copiado entre layouts diferentes costuma sobreviver sem fazer nada** até o dia em que faz.
