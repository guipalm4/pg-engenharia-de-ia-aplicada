...
- [x] **Cheguei a concluir que a Groq reservava `max_tokens` do orçamento. Estava errado, e o teste que me enganou tinha um vício de ordem.** A hipótese vinha de bursts que mostravam menos chamadas passando com `max_tokens` alto — mas eles rodavam com a cota **diária** já no fim, então eu media exaustão de TPD e creditava ao `max_tokens`. Isolando a variável, o resultado se inverte: `max_tokens=60000` **passa** num modelo com teto de 8.000 tokens/minuto (debitando os 113 tokens realmente gerados), e 12 chamadas com `max_tokens=20000` — 240.000 "reservados" contra um teto de 200.000/dia — **passam todas**. O que é debitado é o consumo real. O `Requested = prompt + max_tokens` das mensagens de 429 (`11+2048=2059`, `788+2560=3348`, aritmética exata) é a **checagem de admissão** contra o saldo restante, não o valor cobrado. *Correção:* `max_tokens` voltou a ficar aberto. Capar não economizava nada e cobrava dois preços — recusa mais cedo quando o saldo diário acaba (com o teto aberto o mesmo pedido pesa `Requested=1010` em vez de `3348`) e risco de truncar, que eu já tinha tropeçado ao escolher 2048 sem saber que a maior resposta real era 2.051. **A lição metodológica é a mais cara desta aula: um teste em que os blocos compartilham um recurso que se esgota não compara nada — quem roda depois sempre perde.**
