...
def nl_to_promql(natural_language_query: str):
    ...
    if (
        "taxa de erro" in natural_language_query.lower()
        or "error" in natural_language_query.lower()
    ):
        return 'rate(http_requests_total{status=~"5.."}[5m]) / rate(http_requests_total[5m])'
    if (
        "disco" in natural_language_query.lower()
        or "disk" in natural_language_query.lower()
    ):
