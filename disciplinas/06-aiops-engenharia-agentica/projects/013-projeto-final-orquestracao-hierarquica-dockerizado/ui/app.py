...
with st.sidebar:
    ...
    st.markdown(
        "<h2 style='font-weight:800; margin-top:0;'>NEXUS CONTROL</h2>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='font-size:0.9rem; color:#8a99ad;'>Advanced Agentic AI-Ops & Infrastructure Control Room</p>",
        unsafe_allow_html=True,
    )
    st.markdown("---")
    ...
    st.markdown("---")
    st.markdown(
        "<p style='font-size:0.8rem; color:#6b7280; text-align:center;'>Powered by CrewAI & Streamlit</p>",
        unsafe_allow_html=True,
    )

# ----------------- MAIN HEADER -----------------
st.markdown(
    "<h1 class='title-gradient'>Nexus AI-Ops Platform 🛡️</h1>", unsafe_allow_html=True
)
st.markdown(
    "<p style='font-size:1.2rem; color:#94a3b8; margin-top:-10px; margin-bottom:30px;'>Automação, Segurança e Resiliência em Nuvem sob Governança Inteligente</p>",
    unsafe_allow_html=True,
)
...
with tab_overview:
    ...
    st.write(
        "Conheça os agentes especialistas ativos prontos para receber ordens do Nexus Manager:"
    )
...
with tab_s3:
    ...
    with st.expander("➕ Criar Novo Bucket S3"):
        bucket_name = st.text_input(
            "Nome do Bucket", placeholder="Ex: nexus-finance-data"
        )
    ...
    try:
        ...
        st.error(
            "🔑 Erro de Credenciais AWS. Verifique se o LocalStack está rodando corretamente."
        )
    except Exception as e:
        st.warning(
            f"⚠️ Não foi possível listar os buckets. O LocalStack está de pé na porta 4566? (Erro: {str(e)})"
        )
...
with tab_governance:
    ...
    st.write(
        "Submeta código de infraestrutura para validação em tempo real contra as políticas corporativas da Nexus:"
    )
...
resource "aws_instance" "app_server" {
  ...
  tags = {
    ...
    code_input = st.text_area(
        "Insira seu código Terraform HCL ou YAML", value=example_code, height=250
    )
