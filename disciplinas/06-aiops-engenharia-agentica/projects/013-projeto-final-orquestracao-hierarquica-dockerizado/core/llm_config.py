...
# A Groq usa o MESMO code `rate_limit_exceeded` para duas coisas opostas:
# - "try again in 38.25s"        -> temporal, esperar resolve
# - "please reduce your message  -> a request sozinha estourou o TPM; esperar
#    size and try again"            NUNCA resolve, só encolher o prompt resolve
_PROMPT_GRANDE = re.compile(r"reduce your message size|request too large", re.IGNORECASE)
    ...
    def call(self, *args, **kwargs):
        for tentativa in range(1, MAX_TENTATIVAS + 1):
            try:
                ...
                if _PROMPT_GRANDE.search(str(erro)):
                    print(f"\n🛑 O prompt sozinho estourou o TPM do modelo. Esperar não "
                          f"resolve: encolha o contexto (max_iter/backstories menores) ou "
                          f"aponte para um LLM sem esse teto.\n   {erro}\n")
                    raise
...
nexus_llm = RateLimitAwareLLM(
    ...
    temperature=0.2,
    # Sem isso o manager dispara vários `delegate_work_to_coworker` no MESMO
    # turno; como cada Agent tem um único agent_executor (não reentrante), a
    # segunda chamada ao mesmo coworker volta como "Executor is already running"
    # e o manager perde aquela resposta.
