...
- [x] **Um hack de `sys.path` copiado do material original passou cinco aulas sendo inofensivo e quebrou nesta.** O lab calcula `PROJECT_ROOT = dirname(__file__)/".."` porque os labs do professor vivem em `labs/`. Nas aulas 004–006 isso apontava para o diretório errado e não fazia diferença, porque ninguém usava a variável para montar caminho. Aqui ela é **load-bearing**: `trivy_report_path = os.path.join(PROJECT_ROOT, "data", "trivy.json")`. Copiada como estava, apontaria para um diretório sem `data/` e o pipeline morreria no primeiro `open()`. Corrigido para `os.path.dirname(os.path.abspath(__file__))`. **Código de caminho copiado entre layouts diferentes costuma sobreviver sem fazer nada até o dia em que faz.**
