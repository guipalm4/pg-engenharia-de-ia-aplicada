...
| 07 | [Ferramentas de IA para Gestão de Projetos](#07--ferramentas-de-ia-para-gestão-de-projetos) | 1 projeto |
...
| # | Projeto | O que demonstra |
|---|---------|----------------|
| 001 | [Da Automação à Inteligência Agêntica](disciplinas/06-aiops-engenharia-agentica/projects/001-da-automacao-a-inteligencia-agentica/README.md) | Primeiro agente CrewAI da trilha: um Arquiteto de Cloud que consulta uma tool de políticas de compliance antes de desenhar um bucket S3, em vez de responder de memória — a passagem de IA generativa solta para IA consultiva |
...
| 003 | [Orquestração SRE Assistida por IA](disciplinas/06-aiops-engenharia-agentica/projects/003-orquestracao-sre-assistida-por-ia/README.md) | Entra o terceiro papel, o Engenheiro de SRE: uma Crew de três tasks desenha o manifesto Kubernetes por template, faz o *sync* GitOps chamando `kubectl apply` de verdade — restrito por allowlist de contextos descartáveis e com validação server-side antes de mutar — e decide o rollout analisando métricas de canário contra limiares determinísticos, falhando fechado |
...
| 005 | [Observabilidade Preditiva](disciplinas/06-aiops-engenharia-agentica/projects/005-observabilidade-preditiva/README.md) | Desloca o pipeline no tempo: em vez de diagnosticar o incidente em curso, antecipa-o. Um agente de AIOps encadeia as três camadas de uma plataforma de observabilidade — traduz linguagem natural para PromQL, projeta a série temporal para prever a saturação do volume em 4 horas e materializa o dashboard do incidente em JSON pronto para o Grafana |
| 006 | [ChatOps e Human-in-the-Loop](disciplinas/06-aiops-engenharia-agentica/projects/006-chatops-e-human-in-the-loop/README.md) | Primeira aula com interface: um simulador de Slack em Streamlit onde o agente recebe pedidos de infraestrutura em linguagem natural e precisa exigir aprovação humana antes de executar o que é destrutivo — o padrão *human-in-the-loop* e a assimetria entre um portão determinístico na tool e o julgamento de risco que o modelo faz antes de chamá-la |
| 007 | [DevSecOps com Agentes de IA](disciplinas/06-aiops-engenharia-agentica/projects/007-devsecops-com-agentes-de-IA/README.md) | Primeira aula em que o agente **consome** um artefato de outra ferramenta em vez de gerar o seu: um Analista de DevSecOps lê um relatório de scan de container, separa o backdoor da `xz` (CVE-2024-3094) do ruído e escreve o parecer executivo — triagem de vulnerabilidade como o trabalho que se automatiza, e o julgamento em prosa como o entregável que nenhum validador aceita ou rejeita |
| 008 | [Otimização Inteligente de CI/CD](disciplinas/06-aiops-engenharia-agentica/projects/008-otimizacao-inteligente-de-cicd/README.md) | Um Engenheiro de Platform lê um workflow de GitHub Actions que roda `npm install` do zero a cada push, reescreve o YAML com cache e estima o tempo economizado — com o **enunciado** como objeto de estudo: a dica que entrega o diagnóstico entre parênteses estreita o espaço de solução que o agente explora |
...
## 07 · Ferramentas de IA para Gestão de Projetos

Trilha de *system prompts* para gestão de projetos, percorrendo um caso único — o **RouteWise**, sistema de gestão de frota com 140 veículos — da transcrição de uma reunião de discovery até o portfólio com OKRs. A herança entre os módulos é documental: o artefato produzido em cada um é o insumo do seguinte, e o que se documenta é a iteração sobre o prompt.
