...
| # | Projeto | O que demonstra |
|---|---------|----------------|
| 001 | [Da Automação à Inteligência Agêntica](disciplinas/06-aiops-engenharia-agentica/projects/001-da-automacao-a-inteligencia-agentica/README.md) | Primeiro agente CrewAI da trilha (Arquiteto de Cloud, rodando sobre Groq — modelo definido por `GROQ_MODEL`, default `qwen/qwen3.6-27b`) que consulta uma tool de políticas de compliance antes de desenhar um bucket S3 — passagem de IA generativa solta para IA consultiva |
...
| 006 | [ChatOps e Human-in-the-Loop](disciplinas/06-aiops-engenharia-agentica/projects/006-chatops-e-human-in-the-loop/README.md) | Primeira aula com interface: um simulador de Slack em Streamlit onde o agente recebe pedidos de infraestrutura em linguagem natural e deveria exigir aprovação humana antes de executar o que é destrutivo. O portão determinístico da tool quase nunca é o que decide — o LLM avalia o risco antes de chamá-la, e a senha de aprovação vaza no texto da resposta |
| 007 | [DevSecOps com Agentes de IA](disciplinas/06-aiops-engenharia-agentica/projects/007-devsecops-com-agentes-de-IA/README.md) | Primeira aula em que o agente **consome** um artefato de outra ferramenta em vez de gerar o seu: um Analista de DevSecOps lê um relatório de scan de container, separa o backdoor da `xz` (CVE-2024-3094) do ruído e escreve o parecer executivo. Sem CVSS nem vetor de ataque no fixture, o modelo preenche a lacuna com conhecimento próprio |
| 008 | [Otimização Inteligente de CI/CD](disciplinas/06-aiops-engenharia-agentica/projects/008-otimizacao-inteligente-de-cicd/README.md) | Um Engenheiro de Platform lê um workflow de GitHub Actions que roda `npm install` do zero a cada push, reescreve o YAML com cache e estima a economia. A medição vira um estudo do **enunciado**: a dica dada de presente não muda o diagnóstico, muda a solução — e para pior |
| 009 | [FinOps com Agentes de IA](disciplinas/06-aiops-engenharia-agentica/projects/009-finops-com-agentes-de-IA/README.md) | Primeira aula cujo entregável é um **número**, e não um artefato que um validador aceita ou rejeita: um Consultor de FinOps lê um inventário com um volume EBS órfão, um Elastic IP solto e uma `m5.4xlarge` a 2,5% de CPU, e calcula a economia mensal. O mesmo arquivo devolve quatro totais diferentes |
