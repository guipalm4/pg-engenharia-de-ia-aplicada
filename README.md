...
| 009 | [FinOps com Agentes de IA](disciplinas/06-aiops-engenharia-agentica/projects/009-finops-com-agentes-de-IA/README.md) | Primeira aula cujo entregável é um **número**, e não um artefato que um validador aceita ou rejeita: um Consultor de FinOps lê um inventário de nuvem com um volume EBS órfão, um Elastic IP solto e uma `m5.4xlarge` a 2,5% de CPU, e calcula a economia mensal. Nove execuções sobre o mesmo arquivo devolvem quatro totais diferentes — porque o preço da instância de destino, que responde por 83% da economia, não está no inventário e é gerado a cada volta; a única conta estável é a dos zumbis, a única cujos dois valores vêm do JSON |
| 010 | [RAG e Auto-Remediação](disciplinas/06-aiops-engenharia-agentica/projects/010-rag-e-auto-remediacao/README.md) | Primeira aula em que o agente **consulta a base de conhecimento da empresa** antes de agir: um SRE de Resposta a Incidentes lê o runbook oficial de saturação de conexões no PostgreSQL e escreve o plano de remediação com o post-mortem. O runbook tem 441 bytes e acaba no meio do bloco de diagnóstico — sem seção de remediação —, mas o agente entrega o `pg_terminate_backend` sob o título *"baseado no Runbook Oficial"*, com cláusulas `WHERE` diferentes a cada execução: o que veio do arquivo sai idêntico, o que não veio muda. O RAG não elimina a alucinação, dá a ela a aparência de citação |
